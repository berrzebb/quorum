---
name: quorum:planner
description: "Design tasks into tracks with work breakdowns. Writes and maintains PRDs, generates DRM-driven documents."
---
...
If CPS exists:
1. **CPS.Context → PRD §1** (Problem & Background): Use as-is. The parliament already established the context through multi-round deliberation.
2. **CPS.Problem → PRD §2** (Goals & Success Metrics): Extract gap items as goals. Each gap becomes a measurable acceptance criterion.
3. **CPS.Solution → PRD §4** (Tracks & Requirements): Build items become FR candidates. Buy items become dependency requirements.
4. **CPS.Gaps → Phase 1.5 seed**: Pre-populate the MECE decomposition with gaps already identified by parliament's 5-classification.
5. **Skip Phase 1** if CPS covers the full intent. Proceed directly to Phase 1.5 with CPS-derived actors/systems.
...
**Design Phase is mandatory** for tracks with CPS origin or 3+ WB items. DRM must include Design Phase row with `req` for: Spec (always for API tracks), Blueprint (always for 3+ modules), Domain Model (if persistence involved), Architecture (if infra decisions needed). See `references/design-phase.md`.
