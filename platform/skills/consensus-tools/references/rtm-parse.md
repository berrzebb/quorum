...
```bash
# Parse forward RTM
quorum tool rtm_parse --path docs/plan/track-1/rtm.md

# Filter by requirement ID
quorum tool rtm_parse --path docs/rtm.md --req_id EV-1

# Open items only
quorum tool rtm_parse --path docs/rtm.md --status open

# Backward RTM (test → code → requirement)
quorum tool rtm_parse --path docs/rtm.md --matrix backward

# JSON output with structured row objects
quorum tool rtm_parse --path docs/rtm.md --json
```
