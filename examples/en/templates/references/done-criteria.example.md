# done-criteria example (English)

Copy this file to `templates/references/en/done-criteria.md` and adapt it to your project.

This is the shared "Definition of Done" contract between implementer (Claude) and auditor (GPT/Codex).
- **Done Criteria**: Checklist the implementer verifies before submitting evidence
- **Not-Done Criteria**: Rejection codes the auditor uses when rejecting
- Adjust lint/test commands to match your project tools (e.g., `eslint` → `cargo clippy`, `vitest` → `pytest`)
