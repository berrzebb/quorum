...
1. Added 3 new MCP tools to `core/tools/`:
...
2. Updated hook registrations:
   - `adapters/claude-code/hooks/hooks.json`: 22 → 24 hooks (added ConfigChange, Elicitation)
   - `adapters/gemini/hooks/hooks.json`: 11 → 12 hooks (added BeforeModel)
   - `adapters/codex/hooks/hooks.json`: 5 → 6 hooks (added AfterToolUse)
