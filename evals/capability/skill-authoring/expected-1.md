...
1. **Correct SKILL.md Frontmatter**: The canonical `skills/dependency-audit/SKILL.md` must include YAML frontmatter with at minimum: `name: dependency-audit`, `description` (clear one-line purpose), and `version`. The frontmatter must be valid YAML between `---` delimiters.
...
6. **Protocol-Neutral Inheritance**: The architecture must follow the 3-layer pattern: `agents/knowledge/` (shared protocols) → `skills/dependency-audit/` (canonical implementation) → adapter wrappers (tool bindings only). Domain logic must not be duplicated across adapters.
