...
```bash
node ${ADAPTER_ROOT}/core/audit.mjs {{ arguments }}
```

## Setup

Read config with `read` at `${ADAPTER_ROOT}/core/config.json`:
...
```bash
node ${ADAPTER_ROOT}/core/tools/tool-runner.mjs audit_history --summary
```
