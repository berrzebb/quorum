...
3. **Pre-spawn analysis** (for Tier 2/3 tasks):
   ...
   node ${ADAPTER_ROOT}/core/tools/tool-runner.mjs blast_radius --path . --changed "<task-files>" --json
   # Quality baseline — current state for comparison after implementation
   node ${ADAPTER_ROOT}/core/tools/tool-runner.mjs audit_scan --pattern all --json
   ```
...
All tools: `node ${ADAPTER_ROOT}/core/tools/tool-runner.mjs <tool_name> --json`

## Result Verification

When worker completes:
1. Check worker's evidence via `audit_submit` tool (not main repo)
2. Query verdict from SQLite: `node ${ADAPTER_ROOT}/core/tools/tool-runner.mjs audit_history --summary --json`
