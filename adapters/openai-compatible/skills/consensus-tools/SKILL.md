...
```
${ADAPTER_ROOT}/core/tools/tool-runner.mjs
```
...
The adapter has direct MCP access to quorum tools. Every MCP tool call maps 1:1 to a CLI command:
...
node ${ADAPTER_ROOT}/core/tools/tool-runner.mjs dependency_graph --path src/ --depth 3

# Blast radius of changed files
node ${ADAPTER_ROOT}/core/tools/tool-runner.mjs blast_radius --path src/ --changed "bus/store.ts,bus/lock.ts"

# Performance scan (hybrid: regex + AST)
node ${ADAPTER_ROOT}/core/tools/tool-runner.mjs perf_scan --path src/

# Audit history summary
node ${ADAPTER_ROOT}/core/tools/tool-runner.mjs audit_history --summary --json
```
