...
| 2 | Test (T) | Execute evidence test commands, verify direct tests exist | shell (test runner) |
...
Read `skills/consensus-tools/references/languages.md` for the full coverage matrix and pattern format.
...
Execute the test commands from evidence via `shell`. Verify:
- All referenced tests pass
- Direct test files exist for changed modules (use `find_files`)
