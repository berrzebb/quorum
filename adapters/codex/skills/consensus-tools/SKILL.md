...
If quorum CLI is unavailable, run directly:
```bash
...
Read `skills/consensus-tools/references/languages.md` for the full coverage matrix, pattern format, and how to add new languages.
...
quorum tool rtm_parse --path docs/rtm.md --req_id EV-1
...
For detailed parameters, input/output schemas, and examples for each tool, see: `skills/consensus-tools/references/`
