...
Type safety, test coverage, pattern violations, build health, complexity, security vulnerabilities, dependency risks — these are **objective facts**. quorum measures them with deterministic tools (30 MCP tools, fitness score engine) before any LLM sees the code. If measurable quality drops, the submission is auto-rejected without spending audit tokens.
