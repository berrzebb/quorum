...
function runAutoFix(codes) {
  const fixPromptPath = resolve(HOOKS_DIR, cfg.plugin.fix_prompt);
  ...
  const gptPath = claudePath
    ? resolve(claudePath, "..", plugin.respond_file ?? "gpt.md")
    : "";
  ...
  const prompt = template
    ...
    .replace(/\{\{RESPOND_FILE\}\}/g, gptPath)
    .replace(/\{\{GPT_MD_PATH\}\}/g, gptPath)
